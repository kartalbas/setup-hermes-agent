...
- Traycer drives every coding agent as a subprocess over ACP (Agent Client Protocol, JSON-RPC on stdio): Claude Code, Codex, Cursor, OpenCode, Qwen, Kiro, Kimi, Copilot, Devin, Reasonix, Hermes (`hermes acp` — our agent has that mode), and Antigravity through `agy_acp_server`, a community adapter around the agy CLI (agy itself has no ACP mode; google-antigravity/antigravity-cli issue #31 asks for one). Each CLI brings its own login, hence "subscriptions, not API".
- We already do the same thing: the bridge runs `agy --input-format stream-json --output-format stream-json` (one process, NDJSON events), `opsctl change` runs `claude -p`. ACP would not remove the text tool protocol — an ACP agent stays an agent, not a model for the gateway's tools — and `session/load` is what ADR 0021 gave up on purpose. Not worth switching.
