...
- [ ] Decision pending (operator): rebuild the bridge's tool path on native MCP tools (removes the malformed-call retries, ~half the quota since 7.9.) and re-test the stateful process per conversation with agent mode — ADR before code
