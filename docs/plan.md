...
- [ ] **Module order contradicts a documented dependency.** `install.sh` runs `assistant` (18) before `site` (21), but the Google refresh token is only durable once the consent screen is published, and publishing needs the site's branding URLs to resolve
