...
- [ ] Re-test the stateful process per conversation with agent mode (the operator's point: a standing instance is cheaper — measured 5.9k vs 14.3k new tokens on turn 3): a second bridge with `--stateful` for one bot, one day, watching identity drift and uncached tokens per turn
