...
_assistant_m365_folder() {
    ...
    _assistant_m365_worlds
}

# One sub-folder per world below the root; the share is inherited from it.
_assistant_m365_worlds() {
    [[ -n ${ASSISTANT_M365_WORLDS:-} ]] || return 0
    local IFS=',' entry world out
    for entry in $ASSISTANT_M365_WORLDS; do
        world=${entry%%=*}
        if out=$(runuser -u "$SERVICE_USER" -- "$(assistant_m365ctl)" ensure-folder "${ASSISTANT_M365_ROOT_FOLDER}/${world}" 2>&1); then
...
#   m365ctl status | login | ensure-folder PATH [EMAILS [read|write]] | ensure-mail-rule ALIAS FOLDER | check-mailbox ADDRESS | migrate-worlds [--apply] | companions [--apply] | download PATH LOCAL | upload LOCAL PATH [TEXT_MD_FILE] | move SOURCE TARGET | tools | serve
set -euo pipefail
...
#   googlectl status [ACCOUNT] | login [ACCOUNT] | ensure-folder PATH | migrate-worlds [--apply] | companions [--apply] | download PATH LOCAL | upload LOCAL PATH [TEXT_MD_FILE] | move SOURCE TARGET | tools | serve
set -euo pipefail
